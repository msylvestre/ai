...
function parseResult(dictionnary, data, proposedMapping) {
...
function main() {
  ...
  async.auto({
    ...
    crunchData: ['readCSVHeader', function (result, cb) {
      ...
      cb(null, proposedMapping[0]);

    }],

    parseResult: ['crunchData', function(result, cb) {

      console.log('\n>> Parsing the Results...');
      let aiResult = parseResult(result.buildMyDictionnary.dictionnary, result.readCSVHeader, result.crunchData);
      cb(null, aiResult);
